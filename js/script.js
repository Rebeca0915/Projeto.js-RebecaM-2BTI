...
function carregar() {
  try {
    ...
    if (raw) {
      ...
      // suportar formatos antigos (string ou objeto com text/done)
      listaTarefas = parsed.map((it) => {
        if (typeof it === "string") return { text: it };
        if (it && typeof it.text === "string") return { text: it.text };
...
form.addEventListener("submit", (event) => {
  ...
  if (editandoIndex !== null) {
    ...
    listaTarefas.push({ text: textoFinal });
  }
